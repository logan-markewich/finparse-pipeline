...
3. **Analyze** -- Combine extractions from multiple documents, upload as text to LlamaParse, and extract a cross-document underwriting summary with discrepancy flags
4. **Review** -- Human reviewer sees the analysis, approves or rejects with notes
...
│   │   └── underwriting_summary.py       # UnderwritingSummaryExtraction Pydantic model
│   ├── services/
...
| `POST` | `/reviews/` | Create a review from completed extractions, enqueues cross-doc analysis |
| `GET` | `/reviews/{id}` | Get review (includes underwriting summary) |
...
Three Pydantic models define what to extract. These are pre-built in `app/extraction_schemas/`:
...
**UnderwritingSummaryExtraction** -- cross-document analysis schema: verified income, liquid assets, months of reserves, income sources, asset accounts, discrepancies, notes

## What You'll Implement

Three service files, each with a `TODO` stub and step-by-step instructions in the docstring. Every service uses LlamaParse -- the only external dependency is your `LLAMA_CLOUD_API_KEY`.
...
### 3. `app/services/reviewer.py` -- Review Lifecycle + Cross-Document Analysis

Implement two functions:

`run_review_analysis()` (background job):
- Load linked extractions from the database
- Combine the extracted data into a text document (helper provided: `_format_extractions_as_text`)
- Upload the text as a buffer file to LlamaParse
- Run extraction with the `underwriting_summary` schema
- Store the result on the review record
...
Once all three services are implemented, the full workflow looks like this:
